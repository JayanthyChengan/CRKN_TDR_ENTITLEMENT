...
public class CreateClientTriples extends Object {
	...
	public void run() throws Exception {
		...
		if (l_list != null){
			...
			{
				String g_name = xpath.evaluate("@name", l_list.item(i)).toLowerCase();   
				 ...
				 
				{
					...
					//sp:id				
					semantics_utilities.addIdStatement(g_name.toLowerCase(), organizationResource, organizationModel);
					...
					//sp:id
					Property property = ResourceFactory.createProperty(sp + "name");
	...
	private void addEvent(Resource eventResource , String type) {
		...
		Property dcterms_date = ResourceFactory.createProperty(sp + "date");
	...
	private String getCollectionResource(String collectionname)
	{
		QueryExecution execution = QueryExecutionFactory.create(
				"PREFIX sp: <http://scholarsportal.info/ontologies/core/> "+
						...
						" ?a sp:id '" + collectionname  + "' }", dsg.toDataset()); 
