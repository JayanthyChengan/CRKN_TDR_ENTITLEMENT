...
public class JsonToRdfXml {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Extract triples and add them to the RDF model
            extractTriples(rootNode, "https://ror.org/03dbr7087", model);
    ...
    public static void extractTriples(JsonNode node, String subjectUri, Model model) {
        ...
        while (fieldNames.hasNext()) {
            ...
            if (valueNode.isObject()) {
                // Recursively extract triples for nested objects
                extractTriples(valueNode, subjectUri + "/" + predicate, model);
            } else if (valueNode.isArray()) {
                // Handle arrays by iterating over elements
                for (JsonNode arrayElement : valueNode) {
                    extractTriples(arrayElement, subjectUri + "/" + predicate, model);
                }
            } else if (!valueNode.asText().isEmpty()) {
                // Add triple to the RDF model
                Property prop = model.createProperty("http://scholarsportal.info/ontologies/core/" + predicate);
                //if(! valueNode.asText().equalsIgnoreCase("null")  )
                {
